<?php
	...
	if( !class_exists('Wbcr_FactoryForms461_ListControl') ) {

		class Wbcr_FactoryForms461_ListControl extends Wbcr_FactoryForms461_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo esc_attr($ajax_id); ?>'] = {
							...
							'emptyList': '<?php echo esc_html($this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_461') )); ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_461'));
