 ...
 * и самого Clearfy.
 *
 * $.wbcr_factory_templates_114.app - методы для работы с приложением. Скрыть, показать уведомления.
 * $.wbcr_factory_templates_114.hooks - это иммитация хуков и фильтров аналогично тем, что используются в Wordpress
 *
...
(function($) {
	...
	if( !$.wbcr_factory_templates_114 ) {
		$.wbcr_factory_templates_114 = {};
	}

	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_114.filters = $.wbcr_factory_templates_114.filters || $.wfactory_464.filters;
	//todo: Переопредление для совместимости со старыми версиями плагинов.
	$.wbcr_factory_templates_114.hooks = $.wbcr_factory_templates_114.hooks || $.wfactory_464.hooks;

	$.wbcr_factory_templates_114.app = $.wbcr_factory_templates_114.app || {
		/**
		...
		showNotice: function(message, type) {
			...
			$([document.documentElement, document.body]).animate({
				 ...
				 * Пример регистрации хука $.wfactory_464.hooks.add('wbcr/factory_templates_114/updated',
				 * function(noticeId) {});
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_464.hooks.run('wbcr/factory_templates_114/showed_notice', [noticeId]);
				$.wfactory_464.hooks.run('wbcr/clearfy/showed_notice', [noticeId]);
			});
		...
		hideNotice: function(noticeId) {
			...
			el.fadeOut(500, function(e) {
				 ...
				 * @param {string} noticeId - id уведомления
				 */
				$.wfactory_464.hooks.run('wbcr/factory_templates_114/hidded_notice', [noticeId]);
				$.wfactory_464.hooks.run('wbcr/clearfy/hidded_notice', [noticeId]);
			});
		},
	...
	$.wfactory_464.hooks.add('core/components/pre_activate', function(button) {
	...
	$.wfactory_464.hooks.add('core/components/deactivated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_464.hooks.add('core/components/deleted', function(button) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_464.hooks.add('core/components/activation_error', function(plugin, button, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_114) ) {
			return false;
		}
		...
		if( response.data && response.data.error_message ) {
			$.wbcr_factory_templates_114.app.showNotice(response.data.error_message, 'danger');
		}
	});

	$.wfactory_464.hooks.add('core/components/update_error', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_114) ) {
			return false;
		}
	...
	$.wfactory_464.hooks.add('core/components/activated', function(button, data, response) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !$('#WBCR').length ) {
			return false;
		}
	...
	$.wfactory_464.hooks.add('core/components/ajax_error', function(xhr, ajaxOptions, thrownError) {
		// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
		if( !($('#WBCR').length && $.wbcr_factory_templates_114) ) {
			return false;
		}

		$.wbcr_factory_templates_114.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
	});
